const CACHE_NAME = 'flightscanner-v3.5-cache';
...
self.addEventListener('fetch', (event) => {
  ...
  // Network-first for dynamic flight report data
  if (url.pathname.endsWith('error_fares_report.json')) {
    event.respondWith(
      fetch(event.request)
        .then((response) => {
  ...
  event.respondWith(
    caches.match(event.request).then((cached) => {
      return cached || fetch(event.request).then((response) => {
        if (response.ok && (url.origin === self.location.origin || url.hostname.includes('maptiler'))) {
          const clonedRes = response.clone();
          caches.open(CACHE_NAME).then((cache) => cache.put(event.request, clonedRes));
        }
        return response;
